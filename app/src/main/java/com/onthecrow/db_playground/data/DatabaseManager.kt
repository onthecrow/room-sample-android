...
class DatabaseManager(
    ...
    val prepopulatedState = MutableStateFlow(false)
    ...
    private suspend fun prepopulateDb() {
        if (db.userDao().getOne() != null) {
            prepopulatedState.emit(true)
            return
        }
    ...
    private fun runSyncJob() {
        Log.d("DatabaseManager", "runSyncJob")
        MainScope().launch(Dispatchers.Default) {
            while (true) {
                delay(100)
                mutex.withLock {
                    Log.d("DatabaseManager", "Before changes")
                    db.withTransaction {
                    ...
                    changes.clear()
                    Log.d("DatabaseManager", "Changes processed")
                }
            }
        }
    }
    ...
    private fun runDbModifications() {
        ...
        startPeriodicChange(10, 100) {
            db.userDao().getOne((1_000_000 * Math.random()).roundToInt() - 1)?.let { entity ->
